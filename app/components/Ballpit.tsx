...
class W {
  ...
  update(deltaInfo: { delta: number }) {
    ...
    if (config.controlSphere0) {
      ...
      firstVec.lerp(center, 0.25).toArray(positionData, 0);
      ...
      for (let idx = startIdx; idx < config.count; idx++) {
        ...
        for (let jdx = idx + 1; jdx < config.count; jdx++) {
          ...
          if (dist < sumRadius) {
            ...
            const restitution = 0.3; // Dampen post-collision speed
            const velCorrection = correction.clone().multiplyScalar(Math.max(vel.length(), 0.5) * restitution);
            pos.sub(correction);
            vel.sub(velCorrection);
            pos.toArray(positionData, base);
            vel.toArray(velocityData, base);
            otherPos.add(correction);
            otherVel.add(correction.clone().multiplyScalar(Math.max(otherVel.length(), 0.5) * restitution));
        ...
        if (config.controlSphere0) {
          ...
          if (d < sumRadius0) {
            ...
            const cursorRestitution = 0.3;
            const velCorrection = correction.clone().multiplyScalar(Math.max(vel.length(), 0.5) * cursorRestitution);
            pos.sub(correction);
            vel.sub(velCorrection);
          }
        }
  ...
  setColors(colors: number[]) {
    if (Array.isArray(colors) && colors.length > 1) {
      ...
      for (let idx = 0; idx < this.count; idx++) {
        if (this.config.textColors && idx >= 1 && idx <= this.config.textColors.length) {
          this.setColorAt(idx, new Color(this.config.textColors[idx - 1]));
        } else {
...
function createBallpit(canvas: HTMLCanvasElement, config: any = {}): CreateBallpitReturn {
  ...
  pointerData = createPointerData({
    domElement: canvas,
    onMove(data: any) {
      const activeData = data || pointerData;
      if (!activeData) return;
      raycaster.setFromCamera(activeData.nPosition, threeInstance.camera);
      threeInstance.camera.getWorldDirection(plane.normal);
      raycaster.ray.intersectPlane(plane, intersectionPoint);
      const boost = 2; // try 1.5 → 3
      spheres.physics.center.copy(
        intersectionPoint.clone().multiplyScalar(boost)
      );
  ...
  threeInstance.onBeforeRender = deltaInfo => {
    ...
    if (config.onTextUpdate) {
      config.onTextUpdate(spheres.physics.positionData, threeInstance.camera);
    }
  };
...
const Ballpit: React.FC<BallpitProps> = ({ className = '', followCursor = true, texts = [], ...props }) => {
  ...
  useEffect(() => {
    ...
    spheresInstanceRef.current = createBallpit(canvas, {
      followCursor,
      texts,
      onTextUpdate: (positionData: Float32Array, camera: PerspectiveCamera) => {
        if (!texts.length || !canvas) return;

        // We initialize Vector3 locally or repeatedly, best to just create once. 
        // But since we are inside closure, a new Vector3 per frame is cheap enough, or we use a static one.
        const vec = new Vector3();

        for (let i = 0; i < texts.length; i++) {
          const index = i + 1; // ball 0 is the cursor/interaction ball
          if (index * 3 >= positionData.length) break;

          vec.fromArray(positionData, index * 3);
          vec.project(camera);
          ...
          if (el) {
            // Hide text if it's behind the camera
            if (vec.z > 1) {
              el.style.display = 'none';
            } else {
              el.style.display = 'block';
              el.style.transform = `translate(-50%, -50%) translate3d(${x}px, ${y}px, 0)`;
              // Closer elements have lower vec.z
              el.style.zIndex = Math.round((1 - vec.z) * 1000).toString();
              // Subtly scale smaller if they are far away based on vec.z which goes from -1 to 1.
              // 1 is far, -1 is close. 
              const scale = Math.max(0.3, 1 - (vec.z * 0.5));
  ...
  }, [texts]); // Empty was here, but if texts change, we might want to re-init. Usually texts are constant.

  return (
    <div className="relative w-full h-full">
      <canvas className={`${className} w-full h-full block`} ref={canvasRef} />
      {texts.map((text, i) => (
        <div
          key={i}
          ref={el => { textRefs.current[i] = el; }}
          className="absolute top-0 left-0 pointer-events-none text-white font-black text-3xl md:text-5xl tracking-tight drop-shadow-[0_4px_8px_rgba(0,0,0,1)] whitespace-nowrap will-change-transform"
        >
          {text}
        </div>
      ))}
