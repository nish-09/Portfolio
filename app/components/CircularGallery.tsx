...
class Media {
  ...
  program: any; plane: any; title: any; speed: number = 0;
  width: number = 0; widthTotal: number = 0; x: number = 0; padding: number = 0;
  scale: number = 0; isBefore: boolean = false; isAfter: boolean = false;
...
class App {
  container: HTMLElement; scroll: any; scrollSpeed: number;
  onCheckDebounce: any; renderer: any; gl: any; camera: any; scene: any;
  screen: any; viewport: any; planeGeometry: any; medias: any[] = []; mediasImages: any[] = [];
  raf: number = 0;
  boundOnResize: any; boundOnWheel: any; boundOnTouchDown: any; boundOnTouchMove: any; boundOnTouchUp: any;
  boundOnClick: any;
  isDown: boolean = false; start: number = 0; startX: number = 0;
  lastInteraction: number = Date.now();
  autoScrollSpeed: number = 0.008;
  idleDelay: number = 2000;
  onItemClick: ((index: number) => void) | null = null;

  constructor(container: HTMLElement, { items, bend, textColor = '#ffffff', borderRadius = 0, font = 'bold 30px Figtree', scrollSpeed = 2, scrollEase = 0.05, onItemClick = null }: any = {}) {
  ...
  onTouchUp(e: any) {
    ...
    const moved = Math.abs(endX - this.startX);
    ...
    if (isInside && moved < 8 && this.onItemClick && this.medias.length > 0) {
      ...
      const wrappedIndex = closest.index % originalLen;
      this.onItemClick(wrappedIndex);
    }
    this.onCheck();
  }
  onWheel(e: any) {
  ...
  update() {
    const idle = Date.now() - this.lastInteraction > this.idleDelay;
    if (idle && !this.isDown) {
...
export default function CircularGallery({ items, bend = 3, textColor = '#ffffff', borderRadius = 0.05, font = 'bold 30px Figtree', scrollSpeed = 2, scrollEase = 0.05, onItemClick }: CircularGalleryProps) {
