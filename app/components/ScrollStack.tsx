...
const ScrollStack = ({
  ...
  const getScrollData = useCallback(() => {
    if (useWindowScroll) {
      const scrollTop = sharedLenis?.scroll ?? window.scrollY;
      return {
        scrollTop,
  ...
  }, [useWindowScroll, sharedLenis]);

  const getElementOffset = useCallback(
    (element: HTMLElement) => {
      if (useWindowScroll) {
        const rect = element.getBoundingClientRect();
        return rect.top + (sharedLenis?.scroll ?? window.scrollY);
      }
  ...
  const setupLenis = useCallback(() => {
    ...
    ownsLenisRef.current = true;
    return lenis;
  }, [handleScroll, useWindowScroll, sharedLenis]);
